...
TEST(RedisConfigTest, IndependentClientAndManagementModes) {
  ServerOptions options;
  ...
  EXPECT_EQ(options.client_mode_, lavik::ClientMode::kSingle);
  EXPECT_NE(
      ValidateServerOptions(options).message().find("authority admission"),
      std::string_view::npos);
  EXPECT_FALSE(
      ApplyRedisConfigDirective({"client-mode", "auto"}, &options).ok());
  EXPECT_FALSE(
...
TEST(RedisConfigTest, ConfigRewritePreservesIndependentModes) {
  TempConfigFile config(
      "client-mode cluster\n"
      "meta-managed yes\n"
  ...
  const std::string text((std::istreambuf_iterator<char>(contents)),
                         std::istreambuf_iterator<char>());
  EXPECT_NE(text.find("client-mode cluster"), std::string::npos);
}
