...
with tempfile.TemporaryDirectory(prefix="lavik-dpdk-control-") as directory:
    ...
    with path.open("w") as log:
        child = subprocess.Popen(
            [sys.argv[1]], stdout=log, stderr=subprocess.STDOUT, env=env
        )
        try:
            ...
            while path.read_text().count("bycorf0: Ethernet address:") < 3:
                ...
                time.sleep(0.05)
            subprocess.run(
                ["ip", "link", "set", tap, "address", "02:00:00:00:00:01"], check=True
            )
            subprocess.run(
                ["ip", "addr", "add", "198.18.0.1/24", "dev", tap], check=True
            )
            ...
            print(
                "PASS: Redis listeners only on workers 0/1; outbound DPDK control on worker 2"
            )
        finally:
