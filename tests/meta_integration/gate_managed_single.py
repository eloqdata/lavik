...
def basic_and_stale(root):
    with pair(root, "single-read", client_mode="single") as (meta, source, target, writer):
        ready(meta)
        reader = Client(target)
        try:
            assert reader.call("CONFIG", "GET", "replica-serve-stale-data") == ["replica-serve-stale-data", "yes"]
            ...
            H.wait_until("Single all source flows replay", 20, lambda:
                         reader.call("GET", "count") == "1" and
                         reader.call("HGET", "hash", "field") == "value" and
                         reader.call("SISMEMBER", "set", "member") == 1 and
                         reader.call("LINDEX", "list", 0) == "item" and
                         reader.call("ZSCORE", "sorted", "member") == "1")
            ...
            for command in (("SET", "no-write", "x"), ("INCR", "count"),
                            ("EXPIRE", "ttl", 50), ("PUBLISH", "channel", "x")):
                rejects(reader, command, "READONLY")
            for client in (writer, reader):
                for command in (("MGET", "first-slot", "another-slot"),
                                ("MULTI",), ("SELECT", 1), ("DBSIZE",),
                                ("EVAL", "return 1", 0)):
            ...
            H.wait_until("Single incremental reconnect", 30,
            ...
            meta.pause()
            assert reader.call("CONFIG", "SET", "replica-serve-stale-data", "no") == "OK"
            time.sleep(2)
            assert reader.call("GET", "count") == "2"
            rejects(writer, ("GET", "count"), "MASTERDOWN")
            rejects(writer, ("SET", "count", "bad"), "MASTERDOWN")
            assert reader.call("CONFIG", "SET", "replica-serve-stale-data", "yes") == "OK"
            source.terminate()
            H.wait_until("replication link offline", 15,
            ...
            assert reader.call("CONFIG", "SET", "replica-serve-stale-data", "yes") == "OK"
            assert reader.call("GET", "count") == "2"
            rejects(reader, ("CONFIG", "SET", "replica-serve-stale-data", "maybe"), "yes")
        finally:
            meta.resume()
            reader.close()
    ...
    with process(C.DATA, root / "single-read" / "target", "standalone") as (client, _, _):
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="lavik-managed-single-",
