    ...
    try:
        ...
        if large:
            assert (
                F.redis_call(
                    source,
                    ["CONFIG", "SET", "repl-backlog-size", str(128 * 1024 * 1024)],
                )
                == "OK"
            )
        if case in ("leader-change", "candidate-replace"):
            reply = fixture.leader.putpolicy(
        ...
        staged.write_text(
            "".join(
                f"{node} {flow} {baseline[flow] + count}\n"
        ...
        for node in (candidate, donor1, donor2):
            ...
            H.wait_until(
                "real " + node.node_id[:8] + " vector " + str(counts),
                20,
                lambda node=node, counts=counts: values(node) == list(map(str, counts)),
            )
        for flow in range(2):
            assert (
                source.metric(
                    "lavik_replication_backlog_tail_lsn", f'{{worker="{flow}"}}'
                )
                + 1
                == baseline[flow] + 5
            )
        assert (
            Path(source.log_path).read_text().count("test native event send paused")
            >= 4
        )
        full_counts = {
            node.node_id: Path(node.log_path)
            .read_text()
            .count("durably activated population")
            for node in (candidate, donor1, donor2)
        }
        ...
        if case in ("donor-failure", "leader-change", "candidate-replace"):
            H.wait_until(
                "donor receives real recovery request",
                30,
                lambda: "test recovery donor waiting"
                in Path(donor1.log_path).read_text(),
            )
            if case == "donor-failure":
                donor1.force_kill()
        ...
        if case == "candidate-replace":
            assert len(selected) == 2 and selected[1]["candidate"] == F.FOLLOWER, (
                selected
            )
        ...
        deadlines = {
            (
                record["transition"],
                re.search(r"recovery_deadline_ms=(\d+)", record["detail"])[1],
            )
            for record in records
            if record["event"] == "recovery-start"
        }
        ...
        assert {
            (transition, deadline) for transition, _, deadline in executions
        } == deadlines, executions
        assert len(executions) == (2 if case == "candidate-replace" else 1), executions
        candidate_log = Path(promoted.log_path).read_text()
        completed = re.findall(
            r"candidate recovery completed .*? reason=([^ ]+) applied=([0-9,]+)",
            candidate_log,
        )
        ...
        if case in ("complete", "candidate-replace"):
            assert reason == "target-reached" and recovered == [5, 5], (
                reason,
                recovered,
            )
            ...
            assert reason in ("target-reached", "coverage-unavailable", "deadline"), (
                reason
            )
            ...
            assert reason == "coverage-unavailable" and recovered == [3, 3], (
                reason,
                recovered,
            )
        if case == "complete":
            assert (
                Path(donor2.log_path)
                .read_text()
                .count("test recovery donor rejected first discovery request")
                == 1
            )
            for donor, flow in ((donor1, 0), (donor2, 1)):
                ...
                for delta in (3, 4):
                    assert (
                        f"candidate={F.CANDIDATE} flow={flow} lsn={baseline[flow] + delta}"
                        in log
                    ), log[-10000:]
        for node in alive:
            H.wait_until(
                "reparent preserves recovered data",
                60,
                lambda node=node: values(node) == list(map(str, recovered)),
            )
...
def main():
    ...
    if args.workdir:
        ...
        run(
            root,
            *map(os.path.abspath, (args.meta, args.data, args.ctl)),
            args.mode,
            args.case,
        )
    else:
        with tempfile.TemporaryDirectory(
            prefix="lavik-recovery-", dir=os.environ.get("LAVIK_TEST_DATA_DIR")
        ) as directory:
            run(
                Path(directory),
                *map(os.path.abspath, (args.meta, args.data, args.ctl)),
