...
def normalize_hello(reply):
    ...
    reply = reply.replace(f"${len(match.group(2))}\r\n".encode() + match.group(2) + b"\r\n",
                          b"$8\r\n<server>\r\n", 1)
    reply = reply.replace(f"${len(match.group(3))}\r\n".encode() + match.group(3) + b"\r\n",
                          b"$9\r\n<version>\r\n", 1)
    return reply.replace(b"$2\r\nid\r\n:" + match.group(5) + b"\r\n", b"$2\r\nid\r\n:0\r\n", 1)
    ...
    with tempfile.TemporaryDirectory(prefix="redis-sentinel-contract-") as root:
        for password in ("sentinel-secret", ""):
            ...
            with (Path(root) / "redis.log").open("w") as log:
                process = subprocess.Popen([args.redis_server, str(conf), "--sentinel"],
                                           stdout=log, stderr=log)
                try:
                    deadline = time.monotonic() + 5
                    while True:
                        if process.poll() is not None:
                            raise AssertionError("reference Sentinel exited during startup")
                        try:
                    ...
                    for case in fixture["cases"]:
                        ...
                        if args.record:
                            ...
                            raise AssertionError(f"reference drift in {case['name']}: {replies!r}")
                finally:
                    process.terminate()
                    try:
