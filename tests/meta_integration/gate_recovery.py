...
def scenario_crash_roles(binary, workdir):
    ...
    try:
        ...
        history.check([follower], timeout=30, desc="follower post-crash catch-up")
        ...
        H.log(
            f"node {leader.id} recovered from kill -9 (was leader, "
...
def scenario_snapshot_recovery(binary, workdir):
    ...
    try:
        ...
        history.check([follower], timeout=30, desc="follower snapshot recovery")
...
def scenario_install_snapshot(binary, workdir):
    ...
    try:
        ...
        if snap_idx < last:
            raise H.Failure(f"manual snapshot idx {snap_idx} < last write idx {last}")
        H.log(
            f"node {follower.id} paused; 100 operations + manual snapshot "
        ...
        H.wait_until(
            f"node {follower.id} install_snapshot >= {snap_idx}",
            30,
            lambda: follower.alive() and follower.snapshot_idx() >= snap_idx,
        )
        follower.wait_committed(last, timeout=30)
        history.check([follower], timeout=30, desc="install-snapshot catch-up")
...
def scenario_clean_restart(binary, workdir):
    ...
    try:
        ...
        for node in nodes:
            ...
            H.log(
                f"node {node.id} clean restart: committed {pre} -> {node.committed()}"
            )
...
def main():
    ...
    try:
        for scenario in scenarios:
            ...
            H.log(
                f"{scenario.__name__}: OK ({time.monotonic() - started:.1f}s elapsed)"
            )
        H.log(
            f"PASS ({done}/{len(scenarios)} scenarios) in "
            f"{time.monotonic() - started:.1f}s"
        )
        return 0
    except Exception as exc:  # noqa: BLE001 - dump everything on failure
        print(
            f"[gate-recovery] FAIL after {done}/{len(scenarios)} scenarios: {exc}",
            file=sys.stderr,
        )
