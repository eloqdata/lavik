...
def group_status(fixture, deadline):
    ...
    group = next(
        (item for item in status.get("groups", []) if item.get("group_id") == F.GROUP),
        None,
    )
    if group is None:
...
def wait_group(fixture, description, predicate, timeout=90):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            pass
        time.sleep(0.05)
    raise H.Failure(f"timeout ({timeout}s) waiting for: {description}; group={latest}")


def configure_fast_policies(fixture, suspect_after_ms=1000, lease_duration_ms=1000):
    ...
    fds_before = {data.node_id: data.metric(fds_metric) for data in fixture.data_nodes}
    ...
    if re.fullmatch(r"OK [1-9][0-9]*", lease_reply) is None:
        raise H.Failure(f"fast Authority Lease Policy update failed: {lease_reply}")
    ...
    wait_group(
        fixture, "automatic detector observes healthy Owner", healthy, timeout=30
    )
...
def wait_suspect(fixture, *, reason=None, timeout=20):
    def suspect(group):
        return (
            group.get("automatic_failover_state") == "suspect"
            and int(group.get("suspect_elapsed_ms", "0"))
            < int(group.get("effective_threshold_ms", "0"))
            and (reason is None or group.get("current_reason") == reason)
        )

    return wait_group(
        fixture, "automatic detector enters SUSPECT", suspect, timeout=timeout
    )
...
def wait_successor(fixture, timeout=90):
    def serving(group):
        return (
            group.get("term") == "2"
            and group.get("owner_node_id") in (F.CANDIDATE, F.FOLLOWER)
            and group.get("serving_ready")
        )

    group = wait_group(
        fixture, "automatic transition reaches serving term 2", serving, timeout=timeout
    )
...
def require_successor_write(fixture, successor, scenario):
    ...
    if F.redis_call(fixture.by_id[successor], ["SET", key, "served"]) != "OK":
        raise H.Failure(f"{scenario} successor reported ready but rejected a write")


def require_automatic_begin(fixture, expected_reason=None):
    begin = F.require_unique_failover_event(fixture.metas, "begin", "uncontrolled")
    ...
    if expected_reason is not None and begin["reason"] != expected_reason:
        raise H.Failure(
            f"automatic Begin reason {begin['reason']!r}, want {expected_reason!r}"
        )
...
def run_owner_loss(meta, data, ctl, workdir, mode, require_fault_hook):
    ...
    fixture = F.FailoverFixture(meta, data, ctl, os.path.join(workdir, mode), False)
    try:
        ...
        if mode == "owner-pause":
            ...
            if not rejection.startswith(
                ("CLUSTERDOWN", "TRYAGAIN", "MOVED", "READONLY")
            ):
                raise H.Failure(f"resumed old Owner was not fenced: {rejection}")
            ...
            H.wait_until(
                "resumed old Owner follows successor",
                30,
        ...
        with open(fixture.by_id[successor].log_path, encoding="utf-8") as log:
            if "candidate recovery completed" not in log.read():
                raise H.Failure(
                    "uncontrolled successor prepared without bounded recovery"
                )
        remaining_replica = next(
            node_id for node_id in (F.CANDIDATE, F.FOLLOWER) if node_id != successor
        )
        H.wait_until(
            "remaining replica follows the recovered child history",
            30,
        ...
        if begin["reason"] != expected_reason:
            raise H.Failure(
                f"{mode} committed reason {begin['reason']!r}, want {expected_reason!r}"
            )
        fixture.require_expected_processes_alive(
            dead_data_ids=(F.OWNER,) if mode == "owner-kill" else ()
        )
        H.log(
            f"{mode}: automatic Begin fenced term 1 and successor "
...
def assert_non_overlapping_writes(old_probe, replica_probes, successor):
    ...
    if losing:
        raise H.Failure(f"non-selected replicas accepted writes: {losing}")


def run_partition(meta, data, ctl, workdir, direction, require_fault_hook):
    del require_fault_hook
    name = "one-way-partition" if direction == "downstream" else "two-way-partition"
    fixture = F.FailoverFixture(
        meta, data, ctl, os.path.join(workdir, name), False, proxy_data_control=True
    )
    old_probe = None
    replica_probes = {}
    try:
        ...
        fixture.by_id[F.OWNER].wait_metric(
            ...
            "partitioned Owner self-fences on finite lease expiry",
            timeout=15,
        )
        time.sleep(1.0)
        ...
        if F.redis_call(fixture.by_id[successor], ["SET", key, post_cutover]) != "OK":
        ...
        fixture.require_expected_processes_alive()
        H.log(
            f"{name}: old Owner self-fenced, writes never overlapped, "
...
def kill_logged_meta(fixture, marker, timeout=40):
    ...
    def observed():
        nonlocal paused
        paused = next(
            (node for node in fixture.metas if marker in node.log_tail(lines=1000)),
            None,
        )
...
def run_leader_threshold(meta, data, ctl, workdir, require_fault_hook):
    ...
    try:
        ...
        old_suspect = wait_group(
            fixture,
            "old leader accumulates a material SUSPECT interval",
            lambda group: group.get("automatic_failover_state") == "suspect"
            and group.get("current_reason") == "session_missing"
            and int(group.get("suspect_elapsed_ms", "0")) >= 1500
            and int(group.get("suspect_elapsed_ms", "0"))
            < int(group.get("effective_threshold_ms", "0")),
            timeout=20,
        )
        ...
        old_leader.kill9()
        fixture.leader = H.find_leader(
            [meta for meta in fixture.metas if meta.id != old_leader.id], timeout=20
        )
        wait_group(
            fixture,
            "replacement leader reports observation warmup",
            lambda group: group.get("automatic_failover_state") == "blocked"
            and group.get("blocked_reason") == "leadership_warmup",
            timeout=10,
        )
        ...
        if before.get("term") != "1" or before.get("owner_node_id") != F.OWNER:
            raise H.Failure(f"replacement leader reused prior debounce: {before}")
        successor = wait_successor(fixture)
        ...
        H.log(
            "leader-threshold: replacement discarded "
...
def run_leader_cut(meta, data, ctl, workdir, phase, require_fault_hook):
    ...
    marker = (
        "automatic failover paused before Begin proposal"
        if phase == "proposal"
        else "failover reconciliation paused after automatic uncontrolled Begin group="
    )
    ...
    fixture = F.FailoverFixture(
        meta, data, ctl, os.path.join(workdir, f"leader-{phase}"), require_fault_hook
    )
    try:
        fixture.start_created()
        ...
        if phase == "proposal":
            ...
            if cut.get("term") != "1":
                raise H.Failure(f"pre-proposal leader appended before its cut: {cut}")
        ...
        if phase == "commit":
            H.log(
                "leader-commit: durable transition resumed on replacement "
                f"leader and successor {successor[:8]} served"
            )
        else:
            H.log(
                "leader-proposal: replacement leader performed a fresh "
...
def main():
    args = parse_args()
    binaries = {
        name: os.path.abspath(getattr(args, name)) for name in ("meta", "data", "ctl")
    }
