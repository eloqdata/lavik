...
def server(workers, *, paused=False):
    ...
    args = [
        ...
        "--flush-max-ms=10",
        "--log-dir=" + str(WORKDIR / ("logs-" + str(workers))),
    ]
    if paused:
    ...
    try:
        ...
        print(
            (WORKDIR / "server.log").read_text(errors="replace")[-12000:],
            file=sys.stderr,
        )
        raise
    finally:
...
def check_borrowed_payloads():
    ...
    keys = [
        (b"view-" + str(i).encode() + (b"k" * 5000 if i % 127 == 0 else b""))
    ...
    with server(2) as (client, _):
        ...
        for group in chunks(keys):
            assert client.batch([[b"SET", key, old] for key in group]) == [b"OK"] * len(
                group
            )
    ...
    with server(3) as (client, metrics):
        ...
        for _ in range(2):
            for group in chunks(changed):
                assert client.batch([[b"SET", key, new] for key in group]) == [
                    b"OK"
                ] * len(group)
        ...
        while time.monotonic() < deadline:
            text = (
                urllib.request.urlopen(
                    f"http://127.0.0.1:{metrics}/metrics", timeout=10
                )
                .read()
                .decode()
            )
            completed = int(
                re.search(
                    r'lavik_storage_defrag_runs_total\{result="success"\} (\d+)', text
                ).group(1)
            )
            if completed >= 2:
                break
    ...
    with server(2) as (client, _):
        ...
        for offset in range(0, len(keys), 64):
            ...
            expected = [
                old if i % 5 == 0 else new for i in range(offset, offset + len(group))
            ]
...
def check_payload_crc(obsolete):
    ...
    with server(2, paused=True) as (client, _):
        ...
        for group in chunks(keys):
            assert client.batch([[b"SET", key, new] for key in group]) == [b"OK"] * len(
                group
            )
        changed = [key for i, key in enumerate(keys) if i % 5 != 0]
        for group in chunks(changed):
    ...
    with server(2, paused=True) as (client, metrics):
        ...
        try:
            ...
            at = data.find(old)
            assert at >= 0 and data.find(old, at + 1) < 0, (
                "sentinel not uniquely located"
            )
        ...
        while time.monotonic() < deadline:
            ...
            counts = {
                result: int(count)
                for result, count in re.findall(
                    r'lavik_storage_defrag_runs_total\{result="([^"]+)"\} (\d+)', text
                )
            }
            if (
                counts.get("success", 0) >= 2
                if obsolete
                else counts.get("error", 0) > 0
            ):
                break
            time.sleep(0.1)
        (WORKDIR / "metrics.txt").write_text(text)
    ...
    print(
        "PASS:",
        "obsolete record skipped" if obsolete else "live corruption rejected",
        counts,
        flush=True,
    )
...
def main():
    ...
    with tempfile.TemporaryDirectory(
        prefix="lavik-defrag-payload-", dir=os.environ.get("LAVIK_TEST_DATA_DIR")
    ) as directory:
        root = Path(directory)
