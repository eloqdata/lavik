...
| Storage and recovery | Logical indexes, append/read paths, durable format, shared long-key storage, devices, recovery, shutdown checkpoints, flushing, expiry, and reclamation | [Storage and recovery](04-storage-and-recovery.md), [Grouped collections](09-grouped-collections.md), [Shutdown index checkpoints](06-shutdown-index-checkpoints.md) |
| Streams | Message and consumer-group state, grouped record graph, incremental access and replication contracts | [Streams](12-streams.md) |
| Replication | Native Lavik replication, Redis PSYNC consumption and temporary disk-backed PSYNC export, ScanReader export, destructive full sync, online logs, Sentinel role changes, Meta-managed bounded Candidate Recovery, direct-parent reparent, promotion/follow-owner control, replay, and complete-population Single replica reads | [Replication](05-replication.md) |
...
| Meta control plane | Raft-backed metadata, manifest-bootstrapped multi-member genesis, six durable stores with Topology-owned immutable service mode and lifecycle, per-Group term/owner/authority and failover transitions, atomic creation admission, Data-session publishing, an independent Sentinel Discovery Entry with leader-only topology queries, registered peers and best-effort notifications, cluster status, leader-owned workflow recovery, membership identity, and WAL/snapshots | [Meta control plane](08-meta-control-plane.md) |
