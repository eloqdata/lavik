...
regardless of key length. Auxiliary segment records carry their parent key, so
long keys increase the grouped graph's disk footprint; oversized parent keys
use the existing external-payload rules.
