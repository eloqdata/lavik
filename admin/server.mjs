...
async function body(request) {
  ...
  for await (const chunk of request) {
    if ((length += chunk.length) > 1024 * 1024)
      throw new AdminError("Request exceeds 1 MiB", 413);
    chunks.push(chunk);
  }
  try {
...
export async function start(options = {}) {
  ...
  const directory = resolve(
    options.directory || process.env.LAVIK_ADMIN_DATA || ".lavik-admin",
  );
  await mkdir(directory, { recursive: true, mode: 0o700 });
  const stat = await lstat(directory);
  if (!stat.isDirectory() || stat.mode & 0o077)
    throw new Error(
      "Admin data directory must be a private directory (mode 0700)",
    );
  const tokenPath =
    options.tokenFile ||
    process.env.LAVIK_ADMIN_TOKEN_FILE ||
  ...
  try {
    if (!(await lstat(socketPath)).isSocket())
      throw new Error("Admin socket path is occupied by a non-socket");
    await new Promise((resolve, reject) => {
      const probe = net.connect(socketPath);
      probe.once("connect", () => {
        probe.destroy();
        reject(new Error("Another Lavik Admin owns this data directory"));
      });
  ...
  const cli = net.createServer((socket) => {
    ...
    socket.on("data", async (bytes) => {
      ...
      try {
        ...
        else if (verb === "fleet-status" && id && !a)
          value = await fleet.view(id, true);
        else if (verb === "fleet-operations" && id && !b)
          value = await fleet.operations(id, a || "0");
        else if (verb === "fleet-resume" && id && a && !b)
          value = await fleet.resume(id, a);
        else if (verb === "fleet-abandon" && id && a && !b)
          value = await fleet.abandon(id, a);
        else if (verb === "fleet-failover" && id && a && !b)
          value = await fleet.enqueue(id, "failover", { group: a });
        else if (verb === "fleet-replica-add" && id && a && b && c)
          value = await fleet.enqueue(id, "replica-add", {
            group: a,
            node: b,
            endpoint: c,
          });
        else if (verb === "fleet-replica-remove" && id && a && b && !c)
          value = await fleet.enqueue(id, "replica-remove", {
            group: a,
            node: b,
          });
        else
          throw new AdminError(
            "Use fleet-list, fleet-add NAME SEEDS [PROFILE], fleet-status NAME, fleet-operations NAME [CURSOR], fleet-failover NAME GROUP, fleet-replica-add NAME GROUP NODE ENDPOINT, or fleet-replica-remove NAME GROUP NODE",
          );
        socket.end(`OK ${JSON.stringify(value)}\n`);
      } catch (error) {
        socket.end(`ERR ${JSON.stringify({ error: error.message })}\n`);
      }
    });
  });
  await new Promise((resolve, reject) => {
    cli.once("error", reject);
    cli.listen(socketPath, resolve);
  }).catch(async (error) => {
    await store.close();
    throw error;
  });
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method !== "GET") {
        if (
          request.headers["x-lavik-admin"] !== "1" ||
        ...
        if (!request.headers["content-type"]?.startsWith("application/json"))
          throw new AdminError("JSON content type is required", 415);
      }
      const sessionId = /(?:^|;\s*)lavik_session=([^;]+)/.exec(
      ...
      if (url.pathname === "/api/login" && request.method === "POST") {
        ...
        if (Date.now() - recent.since > 300000) {
          recent.count = 0;
          recent.since = Date.now();
        }
        if (++recent.count > 20)
          throw new AdminError(
            "Too many login attempts; retry in five minutes",
            429,
          );
        loginAttempts.set(ip, recent);
        if (loginAttempts.size > 4096)
          loginAttempts.delete(loginAttempts.keys().next().value);
        if (!equals((await body(request)).token, token))
          throw new AdminError("Invalid access token", 401);
        loginAttempts.delete(ip);
        for (const [key, expiry] of sessions)
          if (expiry <= Date.now()) sessions.delete(key);
        if (sessions.size >= 1000)
          throw new AdminError("Too many active sessions", 503);
        const session = randomBytes(32).toString("base64url");
        sessions.set(session, Date.now() + 8 * 3600000);
        response.setHeader(
          "Set-Cookie",
          `lavik_session=${session}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800${
            process.env.LAVIK_ADMIN_ORIGIN?.startsWith("https:")
              ? "; Secure"
              : ""
          }`,
        );
      ...
      if (url.pathname === "/api/session") {
        send(200, {
          authenticated: true,
          profiles: Object.keys(meta.profiles),
        });
        return;
      }
      if (url.pathname === "/api/logout" && request.method === "POST") {
        ...
        response.setHeader(
          "Set-Cookie",
          "lavik_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0",
        );
