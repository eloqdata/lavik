...
if len(sys.argv) > 2 and sys.argv[1] == "--stop":
    ...
    for pid_file in fixture_root.glob("*.pid"):
        ...
        if (
            command_line.exists()
            and str(fixture_root).encode() in command_line.read_bytes()
        ):
            os.kill(pid, signal.SIGTERM)
            stopped.append(pid)
    deadline = time.monotonic() + 5
    while time.monotonic() < deadline and any(
        Path(f"/proc/{pid}/cmdline").exists()
        and str(fixture_root).encode() in Path(f"/proc/{pid}/cmdline").read_bytes()
        for pid in stopped
    ):
        time.sleep(0.1)
...
def launch(name, args):
    ...
    with (root / f"{name}.log").open("wb") as output:
        process = subprocess.Popen(
            [str(arg) for arg in args], stdout=output, stderr=subprocess.STDOUT
        )
...
def ctl(*args):
    return subprocess.run(
        [str(binary / "lavik-ctl"), *map(str, args)],
        text=True,
        capture_output=True,
        timeout=15,
    )
...
def setup(name, meta_base, data_base, meta_count, groups, replicas, initialize=True):
    ...
    for i in range(meta_count):
        ...
        launch(
            ...
            [
                ...
                i + 1,
                "--addr",
                f"127.0.0.1:{meta_base + i}",
                "--ctl-addr",
                f"127.0.0.1:{meta_base + 100 + i}",
                "--data-control-addr",
                f"127.0.0.1:{meta_base + 200 + i}",
                "--data-dir",
                data_dir,
    ...
    while time.monotonic() < deadline:
        ...
        result = ctl(
            "cluster-status", "--addr", seed, "--allow-plaintext-admin", "--json"
        )
        if result.stdout:
    ...
    while time.monotonic() < deadline:
        ...
        raise RuntimeError(
            f"{name}: did not become ready: {result.stdout} {result.stderr}"
        )
    print(
        f"{name}: ready ({groups} groups, {count} data nodes, {meta_count} Meta members)",
        flush=True,
    )
    return dict(
        id=name,
        seed=seed,
        primary_port=data_base,
        spare=identities[-1],
        spare_endpoint=f"tcp://127.0.0.1:{data_base + count}",
...
try:
    fixtures = (
        [setup("gamma", 8900, 6600, 1, 1, 0, initialize=False)]
        if os.environ.get("LAVIK_ADMIN_FIXTURE_EMPTY") == "1"
        else [setup("alpha", 8100, 6400, 3, 2, 1), setup("beta", 8500, 6500, 1, 1, 0)]
    )
