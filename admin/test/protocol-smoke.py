...
def native(*args, success=True):
    result = subprocess.run(
        [binary, "--addr", seed, *args], capture_output=True, text=True, timeout=15
    )
    if success:
        ...
        assert result.returncode != 0 and result.stdout.startswith("ERR"), (
            result.stdout + result.stderr
        )
...
def fleet(*args):
    result = subprocess.run(
        [binary, "--socket", workspace + "/admin.sock", *args],
        capture_output=True,
        text=True,
        timeout=15,
    )
...
def completed(job):
    ...
    while time.monotonic() < deadline:
        current = next(
            j
            for j in fleet("fleet-operations", "gamma")["jobs"]
            if j["id"] == job["id"]
        )
...
assert "owner-or-transition" in native(
    "unassignnode", "group-1", owner, fields["revision"], success=False
)
assert "stale-revision" in native(
    "unassignnode", "group-1", spare, str(int(fields["revision"]) + 1), success=False
)
assert "not-a-member" in native(
    "unassignnode", "group-1", spare, fields["revision"], success=False
)
...
assert not next(
    n for n in fleet("fleet-status", "gamma")["nodes"] if n["node_id"] == spare
)["group_id"]
...
result = subprocess.run(
    ["redis-cli", "-p", "6600", "GET", "created:in:admin"],
    capture_output=True,
    text=True,
    check=True,
)
...
print(
    "Native protocol passed: pagination, endpoints, primary/CAS guards, add/remove, handshake close, data preservation"
)
