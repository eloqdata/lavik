...
export function decode(buffer, offset = 0, depth = 0) {
  ...
  if (!['$', '*'].includes(kind) || !/^-?\d+$/.test(value)) throw new Error('Invalid RESP reply');
...
export async function command(address, args, profile = {}) {
  ...
  if (!secure && (profile.dataCa || profile.dataCert || profile.dataKey || profile.dataTlsRequired))
    throw new AdminError('This connection profile requires a TLS Data endpoint');
  ...
  const tlsOptions = secure ? {
    ...
    checkServerIdentity: (_, certificate) => tls.checkServerIdentity(target.host, certificate),
  } : {};
  return new Promise((resolve, reject) => {
    const socket = secure ? tls.connect({ ...target, ...tlsOptions }) : net.connect(target);
    let buffer = Buffer.alloc(0);
    let replies = 0;
    const commands = profile.password ? [['AUTH', profile.password], args] : [args];
    const timer = setTimeout(() => finish(new Error('Data request timed out; a write may have committed')), 5000);
    ...
    socket.once('end', () => finish(new Error('Data connection closed before replying')));
    socket.once(secure ? 'secureConnect' : 'connect', () => socket.write(Buffer.concat(commands.map(encode))));
    socket.on('data', chunk => {
      try {
        if (buffer.length + chunk.length > MAX_REPLY) throw new Error('Reply exceeds 8 MiB; narrow the query');
...
export function jsonReply(value) {
  if (Buffer.isBuffer(value)) {
    ...
    return Buffer.from(decoded).equals(value) ? decoded : { base64: value.toString('base64'), bytes: value.length };
  }
...
export function keyBytes(value) {
  if (typeof value !== 'string' || value.length > 65536 || !/^[A-Za-z0-9+/]*={0,2}$/.test(value))
