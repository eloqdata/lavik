...
export function endpoint(value) {
  if (typeof value !== "string")
    throw new AdminError("A numeric IP:port is required");
  const match = /^(?:\[([^\]]+)\]|([^:]+)):(\d+)$/.exec(value);
  if (
    !match ||
    !isIP(match[1] || match[2]) ||
    +match[3] < 1 ||
    +match[3] > 65535
  )
...
export class Meta {
  constructor(binary, profiles = { default: { allowPlaintext: true } }) {
    for (const [name, profile] of Object.entries(profiles)) {
      ...
      const metaTls = [profile.ca, profile.cert, profile.key].filter(
        Boolean,
      ).length;
      if (metaTls !== 0 && metaTls !== 3)
        throw new AdminError(
          `Profile ${name} requires all three Meta TLS files`,
        );
      if (!!profile.dataCert !== !!profile.dataKey)
        throw new AdminError(
          `Profile ${name} requires both Data client certificate and key`,
        );
    }
    this.binary = binary;
  ...
  profile(cluster) {
    ...
    if (!profile)
      throw new AdminError(`Unknown connection profile: ${cluster.profile}`);
    return profile;
  }
  options(cluster, seed, discovery = false) {
    ...
    if (p.ca && p.cert && p.key)
      args.push("--tls-ca", p.ca, "--tls-cert", p.cert, "--tls-key", p.key);
    else if (p.allowPlaintext !== true)
      throw new AdminError("Connection profile requires Meta mTLS");
  ...
  async status(cluster) {
    ...
    for (const seed of JSON.parse(cluster.seeds)) {
      ...
      try {
        ...
        last = result.stderr || "Meta returned an invalid status response";
      }
    }
  ...
  async leader(cluster) {
    ...
    const leader = status.meta_members.find(
  ...
  async nodes(cluster, status) {
    ...
    for (let i = 0; i < status.data_nodes.length; i += 8) {
      nodes.push(
        ...(await Promise.all(
          status.data_nodes.slice(i, i + 8).map(async (node) => {
            try {
              const reply = await this.command(
                cluster,
                ["getnode", node.node_id],
                leader,
              );
