...
- 每点仅一轮，峰值为连接数扫描的最大值，小幅差异不代表统计显著。产品的内存预算、缓存、磁盘拓扑和持久性语义不同；不能跨 10M / 1B 两组混排，也不能声称等持久性成本。上一轮 Lavik 为 97f63d0 且暂停 defrag，本轮为 3955b98 且开启 defrag，两轮变化不能只归因于 defrag。详细方法见英文版。
